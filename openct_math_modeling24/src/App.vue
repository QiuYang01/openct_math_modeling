<template>
  <div class="row" id="row" v-loading="loading1">
    ...
    <div class="right" id="right">
        ...
        <div class="qytip"> 
          提示:<br />
          1.点击右下方<b>答题区</b>利用6个步骤完成此题；<br />
          2.点击<b>总览区</b>表格第一行可修改顺序；<br />
          3.点击<b>总览区</b>表格内容可修改答案<br />
          4.作答时间达75分钟自动提交<br />
        </div>
    ...
    <div class="left" id="left">
      ...
      <div class="resulttable">
        <table class="table">
            ...
            <td v-for="(step,indexqy) in result.dataJson" :key="step.id">
              <tr>
                <el-tooltip placement="top">
                  <div slot="content">
                    <el-button :disabled="indexqy==0" type="text" @click="datajsonsortdown(step,indexqy)">左移</el-button>
                    <el-button :disabled="indexqy==result.dataJson.length-1" type="text" @click="datajsonsortup(step,indexqy)">右移</el-button>
                  </div>
                  <el-button type="text" style="color:#fff;width: 84px;text-align:center">{{step.context}}</el-button>
                </el-tooltip>
              </tr>
              <!-- <tr>
                <el-tooltip :open-delay="100" class="item" effect="dark" placement="top-start">
                  <div slot="content" >1</div>
                  <el-button style="border-radius:0">{{step.context}}</el-button>
                </el-tooltip>
              </tr> -->
              <tr @click="alertoption(step.context,1,step.id,'a')">
                 <el-tooltip :open-delay="500" class="item" effect="light" placement="top-start">
                  <div slot="content" v-html="step.a"></div>
                  <el-button :type="finishedabcd(step,'a')?'success':'info'">{{finishedabcd(step,'a')?'已完成':'未完成'}}</el-button>
                </el-tooltip>
              </tr>
              <tr @click="alertoption(step.context,2,step.id,'b')">
                 <el-tooltip :open-delay="500" class="item" effect="light" placement="top-start">
                   <div slot="content" v-html="step.b"></div>
                  <el-button :type="finishedabcd(step,'b')?'success':'info'">{{finishedabcd(step,'b')?'已完成':'未完成'}}</el-button>
                </el-tooltip>
              </tr>
              <tr @click="alertoption(step.context,3,step.id,'c')">
                 <el-tooltip :open-delay="500" class="item" effect="light"  placement="top-start">
                   <div slot="content" v-html="step.c"></div>
                  <el-button :type="finishedabcd(step,'c')?'success':'info'">{{finishedabcd(step,'c')?'已完成':'未完成'}}</el-button>
                </el-tooltip>
              </tr>
              <tr @click="alertoption(step.context,4,step.id,'d')">
                 <el-tooltip :open-delay="500" class="item" effect="light"  placement="top-start">
                   <div slot="content" v-html="step.d"></div>
                  <el-button :type="finishedabcd(step,'d')?'success':'info'">{{finishedabcd(step,'d')?'已完成':'未完成'}}</el-button>
                </el-tooltip>
              </tr>
      ...
      <div class="leftBottom">
        <div class="steps" v-if="alertcurrentOpotion.length">
          ...
          <el-button  @click="alertStepOption()" type="success" >确认修改</el-button>
        </div>
        <div v-else>
            <!-- 显示步骤{{result}} -->
            <div class="steps" v-if="showsteps">
              ...
              <p v-if="result.dataJson.length==6">点击上方表格可修改选项，若完成所有选项，则可点击下方按钮提交</p>
...
export default {
  ...
  methods:{
    // 步骤左移
    datajsonsortdown(item,indexqy){
      // console.log(indexqy);
      if(indexqy<=0){
        this.errMsg("不能左移了");
        return
      }
      const temp = this.result.dataJson[indexqy-1];
      this.result.dataJson[indexqy-1] = this.result.dataJson[indexqy];
      this.result.dataJson[indexqy] = temp;
      console.log(this.result.dataJson)
      this.$forceUpdate()
    },
    // 步骤右移
    datajsonsortup(item,indexqy){
      if(indexqy>=5){
        this.errMsg("不能右移了");
        return
      }
    ...
    //点击表格后修改小选项 
    alertoption(context,area,curAlertstepId,abcd){ //通过两个字段在24个找出选项 curAlertstepId为当前修改的结果的step.id
      //元数据统计 修改次数
      this.result.answer2++;
    ...
    alertchangoptionindex(area,options){
       //找回之前修改的
       console.log("alertchangoptionindex",options,area,)
    ...
    submitResult(){
        ...
        // console.log(this.enterpageTime)
  ...
  mounted(){
    ...
    this.questionId =  "24"; //2021-10-08修改成静态的
    ...
    //75分钟自动提交
    setTimeout(()=>{
      this.$confirm('你已经作答75分钟即将自动提交', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '确定',
        type: 'warning'
      })
